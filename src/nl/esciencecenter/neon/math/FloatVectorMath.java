...
public final class FloatVectorMath {
    ...
    public static Float4Vector[] bezierCurve(int steps, Float4Vector startLocation, Float3Vector startControl,
        ...
        for (int i = 0; i < steps; i++) {
            newBezierPoints[i] = new Float4Vector(0f, 0f, 0f, 0f);
        }
    ...
    public static Float3Vector RGBtoHSV(Float3Vector rgb) {
        float r = rgb.getX(), g = rgb.getY(), b = rgb.getZ();
    ...
    public static Float3Vector HSVtoRGB(Float3Vector hsv) {
        float h = hsv.getX(), s = hsv.getY(), v = hsv.getZ();
        int i;
        float f, p, q, t, r, g, b;
        if (s == 0) {
            // achromatic (grey)
            r = g = b = v;
            return new Float3Vector(r, g, b);
        }
        h /= 60; // sector 0 to 5
        i = (int) Math.floor(h);
    ...
    public static Float3Vector increaseHue(Float3Vector rgb, float factor) {
        Float3Vector hsv = RGBtoHSV(rgb);
        return HSVtoRGB(new Float3Vector(hsv.getX() * factor, hsv.getY(), hsv.getZ()));
    }

    public static Float3Vector increaseSaturation(Float3Vector rgb, float factor) {
        Float3Vector hsv = RGBtoHSV(rgb);
        return HSVtoRGB(new Float3Vector(hsv.getX(), hsv.getY() * factor, hsv.getZ()));
    }

    public static Float3Vector increaseValue(Float3Vector rgb, float factor) {
        Float3Vector hsv = RGBtoHSV(rgb);
        return HSVtoRGB(new Float3Vector(hsv.getX(), hsv.getY(), hsv.getZ() * factor));
    }
}
